...
public class FullMap extends ScreenAdapter implements InterfacciaComune {
    ...
    // quando entra la mette a true e fino a quando non termina la battaglia rimane
    // a true, quando termina la battaglia viene rimessa
    // a false e viene salvato il dato sul json e anche sul bot. (momentaneamente
    ...
    public void show() {
        try {
            ...
            int mapWidth = mappa.getProperties().get("width", Integer.class)
                    * mappa.getProperties().get("tilewidth", Integer.class);
            int mapHeight = mappa.getProperties().get("height", Integer.class)
                    * mappa.getProperties().get("tileheight", Integer.class);
            ...
            System.out.println("Errore show fullMap, " + e);
        }
    ...
    public void render(float delta) {
        ...
        if (battagliaIsFinished && isNormalBot) {
            ...
            if (inEsecuzione == false) {
                if (game.getLuogo().equals("percorso1") || game.getLuogo().equals("bosco")
                        || game.getLuogo().equals("percorso2") ||
            ...
            System.out.println("Errore render fullMap, " + e);
        }
    ...
    private void cambiaProfondita(MapLayer lineeLayer, MapLayer lineeBloccaStoria) {
        ...
        for (MapObject obj : lineeLayer.getObjects()) {
            ...
            for (Bot bot : botListBack) {
                game.renderPersonaggiSecondari(bot.getCurrentAnimation(), bot.getPosition().x, bot.getPosition().y,
            ...
            System.out.println("Errore cambiaProfondita fullMap, " + e);
        }
    ...
    private void renderLayer(String layerName) {
        try {
            ...
            System.out.println("Errore renderLayer FullMap, " + e);
        }
    ...
    private String findGroupByLayerName(TiledMap map, String layerName) {
        try {
            ...
            System.out.println("Errore findGroupByLayerName FullMap, " + e);
            return null;
        }
    ...
    private MapLayer findLayerInGroup(TiledMap map, String groupName, String layerName) {
        try {
            ...
            System.out.println("Errore findLayerInGroup fullMAp, " + e);
            return null;
        }
    ...
    private void checkInteractionBot() {
        try {
            ...
            System.out.println("Errore checkInteractionBot fullMap, " + e);
        }

    }

    // metodo che fa muovere il bot verso il player e per disegnare il punto
    // esclamativo
    private void triggerBot(Bot bot) {
        ...
        try {
            ...
            System.out.println("Errore triggerBot fullMap, " + e);
        }
    ...
    private boolean checkOverlaps(Bot bot) {
        try {
            ...
            // Controlla se il rettangolo del personaggio è completamente contenuto nel
            // rettangolo del bot
            ...
            System.out.println("Errore checkOverlaps fullMap, " + e);
            return false;
        }
    ...
    private void muoviBot(Bot bot) {
        ...
        try {
                ...
                case "-y":
                    ...
                    if (torna) {
                        ...
                        if (positionY < yBase) {
                            ...
                            // da mettere a false alla fine della battaglia (considerare di bloccare quel
                            // bot se finisce la battaglia prima)
                            inEsecuzione = false;
            ...
            System.out.println("Errore muoviBot fullMap, " + e);
        }
    ...
    private void renderizzaDiscorsoInizio() {
        try {
            ...
            System.out.println("Errore renderizzaDiscorsoInizio fullMap, " + e);
        }
    ...
    private void fineBattagliaNuotatore() {
        try {
            ...
            System.out.println("Errore fineBattagliaNuotatore fullMap, " + e);
        }
    ...
    public void checkFerma() {
        try {
            ...
            System.out.println("Errore checkFerma FullMap, " + e);
        }
    ...
    private void muoviNuotatore(Bot bot) {
        try {
            ...
            System.out.println("Errore muoviNuotatore fullMap, " + e);
        }

    }

    // -------------------------SETTAGGO POSIZIONI E CONTROLLO
    // LUOGO---------------------------------------
    public void checkInAcqua() {
        try {
            ...
            System.out.println("Errore checkInAcqua fullMap, " + e);
        }
    ...
    public void checkLuogo() {
        try {
            ...
            System.out.println("Errore checkLuogo fullMap, " + e);
        }
    ...
    public void setPositionPlayer() {
        try {
            ...
            System.out.println("Errore setPositionPlayer fullMap, " + e);
        }
    ...
    private void setPositionBot() {
        try {
                ...
                if (object instanceof RectangleMapObject) {
                    ...
                    if (object.getProperties().get("path") != null) {
                        ...
                        if (object.getProperties().get("nuota") == null) {
                            ...
                            // creo un nuovo bot andando a prendere la path dello spritesheet dalla sua
                            // proprietà
                        ...
                        // bot in acqua
                            ...
                            Bot bot = new Bot(24, 24, (String) object.getProperties().get("path"), 35, 60,
                                    convertStringToFloat((String) object.getProperties().get("nuota")), rect.getX(),
                                    rect.getY());
                            bot.setPosition(rect.getX(), rect.getY());
                            bot.setXbase(rect.getX());
                            bot.setYbase(rect.getY());
            ...
            System.out.println("Errore setPositionBot fullMap, " + e);
        }
    ...
    private void salvaBotJson(MapObject object, Bot bot) {
        try {
            ...
            System.out.println("Errore salvaBotJson fullMap, " + e);
        }
    ...
    private void setBlocca(MapLayer rettangoliBlocca, Bot bot, RectangleMapObject rectObject) {
        try {
            ...
            System.out.println("Errore setBlocca fullMap, " + e);
        }
    ...
    private void gira(Bot bot, String direzione) {
        try {
            ...
            System.out.println("Errore gira fullMap, " + e);
        }
    ...
    private void giraNuotatore(Bot bot, String direzione) {
        try {
            ...
            System.out.println("Errore giraNuotatore fullMap, " + e);
        }
